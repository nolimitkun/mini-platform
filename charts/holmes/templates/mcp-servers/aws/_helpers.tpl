...
1. **Avoid inline JSON — prefer shorthand that needs no JSON.** For a breakdown by a dimension use `--group-by Type=DIMENSION,Key=SERVICE` (no quotes to lose), then pick out the row you care about yourself in your analysis (e.g. keep only the row whose service key is "Amazon Bedrock"). This is the most reliable approach and sidesteps the quoting problem entirely. Example:
   ...
   **Do NOT also pass `--filter` when you use `--group-by`.** The group-by output already contains every value of that dimension — read the one you want from it. Combining `--group-by` with a JSON `--filter` is the #1 cause of failed cost queries; if you find yourself writing both, delete the `--filter`.
2. **If you truly need a JSON `--filter`** (rare — try option 1 first), the value must satisfy BOTH rules at once, or it breaks: (a) enclose the ENTIRE value in double quotes, and (b) backslash-escape every inner double quote. Correct: `--filter "{\"Dimensions\":{\"Key\":\"SERVICE\",\"Values\":[\"Amazon Bedrock\"]}}"`. The leading `"` and trailing `"` are mandatory — without them the value is split on the first space (e.g. `Amazon Bedrock` splits into two arguments) and AWS receives truncated JSON. Escaping the inner quotes alone is NOT enough (`--filter {\"Dimensions\":...}` fails exactly this way). If it still fails, stop retrying variations and use option 1.
