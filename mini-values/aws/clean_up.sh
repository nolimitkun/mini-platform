...
mount_targets=$(aws efs describe-mount-targets --region "$REGION" --file-system-id "$fs_id" --query "MountTargets[*].MountTargetId" --output text)

# Delete each mount target
for mt_id in $mount_targets; do
    echo "Deleting Mount Target: $mt_id"
    aws efs delete-mount-target --region "$REGION" --mount-target-id "$mt_id"
done

# Wait for mount targets to be deleted (optional, prevents API conflicts)
while [[ -n $(aws efs describe-mount-targets --region "$REGION" --file-system-id "$fs_id" --query "MountTargets[*].MountTargetId" --output text) ]]; do
...
for sg in $(aws ec2 describe-security-groups --region "$REGION" --filters "Name=group-name,Values=efs-sg" --query "SecurityGroups[*].GroupId" --output text); do

    echo "Deleting Security Group: $sg"
    for eni in $(aws ec2 describe-network-interfaces --region "$REGION" --filters "Name=group-id,Values=$sg" --query "NetworkInterfaces[*].NetworkInterfaceId" --output text); do
        echo "$eni"
...
aws cloudformation delete-stack \
    --stack-name eksctl-production-stack-nodegroup-gpu-nodegroup
aws cloudformation delete-stack \
    --stack-name eksctl-production-stack-cluster
